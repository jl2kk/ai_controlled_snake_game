...
class SnakeGame:
    ...
    def is_collision(self):
        ...
        self_collision = self.head in self.snake[1:]
        return hit_boundary or self_collision
    ...
    def ai_move(self):
        ## TO DO
        # x_diff = self.food.x - self.head.x
        # y_diff = self.food.y - self.head.y

        # if abs(x_diff) > abs(y_diff):
        #     self.direction = Direction.RIGHT if x_diff > 0 else Direction.LEFT
        # else:
        #     self.direction = Direction.DOWN if y_diff > 0 else Direction.UP
